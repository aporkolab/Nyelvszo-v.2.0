...
router.get('/', catchAsync(async (req, res) => {
  ...
  try {
    ...
    if (dbStatus === 1) {
      ...
      healthCheck.database.collections = await mongoose.connection.db.listCollections().toArray().then(collections => collections.length);
    }
  } catch (error) {
    healthCheck.database = {
      status: 'error',
...
router.get('/detailed', catchAsync(async (req, res) => {
  ...
  logger.audit('Detailed health check performed', {
    ...
    unhealthyChecks: Object.values(detailedHealth.checks).filter(check => check.status === 'unhealthy').length
  });
